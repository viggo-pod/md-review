...
- **Scenario-aware weighted dimensions** — scenario reviews use Logic 30%, Scenario completeness 25%, Sections 15%, References 10%, Redundancy 10%, Format 10%; generic reviews omit scenario completeness and normalize the remaining five dimensions to 100%. Each dimension score is the ratio of satisfied items in its checklist or rule index (counted per applicable item) × 100.
...
- **Self-validating** — ships 12 count-based evals, test fixtures and templates, a trigger-query set, and a one-command regression harness (`bash evals/run_self_test.sh`).
