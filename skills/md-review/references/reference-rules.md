...
Apply reference rules only to references present in the document or to the selected internal-reference checks. A rule with at least one finding = 1 unmet item (occurrence counts reported as severity), and a rule with no finding is satisfied. Mark absent link, image, or cross-reference constructs **N/A** with a reason. Dimension score = (applicable rules with no finding) ÷ (applicable rules) × 100; N/A rules are excluded from both numerator and denominator.
