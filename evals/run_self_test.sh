...
echo ""
echo "[2/5] Precision & sensitivity baselines (local reports, when present)"
PRECISION_ALL_PASS=1
PRECISION_REPORTS_FOUND=0
for CLEAN in clean-api clean-prd clean-gdd; do
  CLEAN_REPORT="$SKILL_DIR/evals/reports/${CLEAN}-report.md"
  if [ -f "$CLEAN_REPORT" ]; then
    PRECISION_REPORTS_FOUND=$((PRECISION_REPORTS_FOUND+1))
...
if [ "$PRECISION_REPORTS_FOUND" -eq 3 ] && [ "$PRECISION_ALL_PASS" -eq 1 ]; then
  ok "clean docs (api/prd/gdd): score>=85 and no P0 (precision baseline)"
elif [ "$PRECISION_REPORTS_FOUND" -lt 3 ]; then
  echo "  SKIP  clean-doc precision baseline — local reports are not published"
else
...
if [ "$SENS_REPORTS_FOUND" -eq 3 ] && [ "$SENS_ALL_PASS" -eq 1 ]; then
  ok "defect docs (prd/api/gdd-test): rejected by gate (P0>=1 or score<75)"
elif [ "$SENS_REPORTS_FOUND" -lt 3 ]; then
  echo "  SKIP  defect sensitivity baseline — local reports are not published"
else
