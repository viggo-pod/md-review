...
An invalid scenario value (not one of the 21 defined scenarios) must **not** fall back to a generic review. Per SKILL.md Error Handling, the reviewer lists the 21 valid values — `prd, adr, add, api, brd, mrd, fsd, gdd, gdo, tdd, ldd, concept, intent, capability, feature, research, tld, tcd, decq, issue, gpr` — and exits with code **2**, so a typo can never silently skip the scenario-completeness dimension.
