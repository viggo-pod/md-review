...
| `capability` | 能力分析文档 | 能力编号、输入输出、失败路径、依赖、缺口影响与解决方案 |
| `feature` | Feature Analysis 文档 | 版本化功能清单、验收、蓝本、来源、交接 |
| `research` | 领域借鉴/研究分析 | 来源证据、借鉴映射、许可证边界、置信度、交接 |
| `tld` | 任务清单文档 | 任务拆解粒度、依赖、工作量估算、负责人 |
| `tcd` | 测试用例文档 | 正向、边界、异常覆盖、前置条件、可验证预期结果、需求追溯、测试用例集 |
| `decq` | Durable Decision Queue（持久化决策队列） | 持久化决策输入、人工交接、open/closed 状态、ADR 链接、机器可恢复与路由 |
...
| `--format full` | 完整报告（默认） |
| `--format summary` | 评分表加 `MD-REVIEW-SUMMARY` 交接块，不含问题详情 |
...
- `evals/docs/` —— 23 份夹具：14 份注入缺陷的场景夹具、2 份通用缺陷夹具、1 份通用审查夹具、3 份干净文档，以及 3 份边界夹具（二进制、非 UTF-8、编号断裂）。
- `evals/evals.json` —— 12 条计数制评测定义（3 个缺陷场景审查、1 个 generic 审查、3 个干净文档精度检查、错误路径、步骤编号、模糊规则、敏感信息、路径门禁）。
