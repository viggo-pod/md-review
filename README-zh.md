...
单次调用审查一份 Markdown 文档，覆盖 17 种文档场景（PRD、API 规范、GDD、TDD、ADR 等），或使用通用模式。

## 核心特性

- **Bug 优先定位** —— 逻辑（30%）是最高权重维度：公式/数字矛盾、缺失边界情况、断裂流程。无论聚焦哪些维度，P0（阻断级）问题都会被报告。
- **17 种场景** —— 每种场景带各自的必需内容检查清单（验收标准、5W1H、接口契约、错误码、测试用例等）。
...
- **自带验证** —— 内置 12 条计数制 evals、8 份参考报告、注入缺陷夹具、触发问句集与一条命令回归（`bash evals/run_self_test.sh`）。
...
- `bash evals/run_self_test.sh` —— 回归框架：脚本化检查(`verify_scripts.py` 功能点验证、注册表完整性)始终运行；基于 agent 的检查对照参考报告验证——干净文档精度(≥85、P0=0)、缺陷夹具敏感性(被拒绝：P0≥1 或 <75)、错误路径协议、步骤编号检测。
...
- `evals/reports/` —— 8 份参考报告：3 份干净文档精度、3 份缺陷夹具敏感性、错误路径协议、步骤编号检测。
