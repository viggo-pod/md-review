...
- **6 个加权维度** —— 逻辑 30%、场景完整性 25%、章节 15%、引用 10%、冗余 10%、格式 10%。**总分 = Σ(维度分 × 权重)**，百分制。
- **CI 就绪** —— `--solo` 模式带退出码门禁（`--pass-threshold`，默认 75），每份报告末行输出机器可读的 `MD-REVIEW-SUMMARY` 块。
- **自带验证** —— 内置评测套件（注入缺陷夹具、干净文档精度基线、错误路径协议、步骤编号检测），一条命令回归（`bash evals/run_self_test.sh`）。
...
- `bash evals/run_self_test.sh` —— 5/5 回归检查：脚本功能点验证、干净文档精度基线（clean PRD/API/GDD 无误报）、错误路径协议（缺失/二进制/非 UTF-8 → exit 2）、注册表完整性、步骤编号断裂检测。
- `evals/docs/` —— 21 份夹具：15 份注入缺陷的场景文档、3 份干净文档，外加二进制/非 UTF-8/编号断裂边界用例。
- `evals/evals.json` —— 10 条评测定义（场景审查、干净文档精度、错误路径、编号断裂）。
- `evals/reports/` —— 干净文档与协议边界用例的参考报告（clean-api 95.0、clean-prd 93.3、clean-gdd 93.3，均 0 P0）。
